...
import putRequestsBody from "../../../sampleUtils/putRequestbody.json";
//write test
test("create PUT API request using playwright", async ({ request }) => {
  ...
  */
  const putRespons = await request.put(`/posts/${bId}`, {
    data: putRequestsBody,
  });
  const putResponsebody = await putRespons.json();
  console.log("=======Put API RESPONSE============", putResponsebody);

  //validate status code
  expect(putRespons.status()).toBe(200);
});
