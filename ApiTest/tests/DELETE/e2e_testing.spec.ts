...
test("create Delete API request using playwright", async ({ request }) => {
  ...
  */
  ...
  //validate status code
  await expect(patchRespons.status()).toBe(200);

  console.log("===================== DELETE API =========================");
  //Delete api we have to find header and token number as done for put,
  const deleteApiResponce = await request.delete(`/posts/${bId}`);

  //validate status code and text
  await expect(patchRespons.status()).toBe(200);
  console.log("The retun text is", await patchRespons.statusText());
  await expect(patchRespons.statusText()).toBe("OK");
});
